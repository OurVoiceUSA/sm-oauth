...
// General Auth0 strategy, could be used for a universal login page.
...
// Auth0 client configured for only SMS login, to enable SMS-specific login button.
const passport_auth0_sms = {
   domain: getConfig("oauth_auth0_sms_domain", false, null),
   clientID: getConfig("oauth_auth0_sms_clientid", false, null),
   clientSecret: getConfig("oauth_auth0_sms_secret", false, null),
   callbackURL: ov_config.wsbase+'/auth/auth0_sms/callback',
};

// Auth0 client configured for only Passworkd login, for user/pass login button.
const passport_auth0_password = {
   domain: getConfig("oauth_auth0_password_domain", false, null),
   clientID: getConfig("oauth_auth0_password_clientid", false, null),
   clientSecret: getConfig("oauth_auth0_password_secret", false, null),
   callbackURL: ov_config.wsbase+'/auth/auth0_password/callback',
};
...
function moauthredir(req, res) {
  ...
  if (req.session.aud && !req.session.aud.match(/\.ourvoiceusa\.org/)) {
    redir = 'https://'+req.session.aud;
  }
  if (req.session.app) {
    context = 'web';
...
export function doExpressInit(log, redis) {
  ...
  // otp-sms react app
  app.use('/auth/otp-sms/', express.static(ov_config.dir_otp_sms));
  ...
  if (passport_auth0_sms.clientID && passport_auth0_sms.clientSecret && passport_auth0_sms.domain) {
    passport.use(new Auth0Strategy(passport_auth0_sms,
      async (accessToken, refreshToken, profile, done) => done(null, transformAuth0Profile(profile._json))
    ));
    app.get('/auth/sms', function(req, res, next) {
      req.session.aud = req.query.aud;
      req.session.device = req.query.device;
      passport.authenticate('sms', { callbackURL: ov_config.wsbase+'/auth/sms/callback' }
      )(req, res, next)});
    app.get('/auth/sms/callback', passport.authenticate('auth0_sms', { failureRedirect: '/auth/sms' }), moauthredir);
  }

  if (passport_auth0_password.clientID && passport_auth0_password.clientSecret && passport_auth0_password.domain) {
    passport.use(new Auth0Strategy(passport_auth0_password,
      async (accessToken, refreshToken, profile, done) => done(null, transformAuth0Profile(profile._json))
    ));
    app.get('/auth/password', function(req, res, next) {
      req.session.aud = req.query.aud;
      req.session.device = req.query.device;
      passport.authenticate('password', { callbackURL: ov_config.wsbase+'/auth/password/callback' }
      )(req, res, next)});
    app.get('/auth/password/callback', passport.authenticate('password', { failureRedirect: '/auth/password' }), moauthredir);
  }
